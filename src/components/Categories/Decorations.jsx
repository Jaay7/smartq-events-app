    ...
    <div className={classes.main}>
      {
        ...
        data.error ? 'Error' :
        <>
          <div className={classes.banner} style={{backgroundImage: `url(${data.data.extras.categories.Decorations.bannerImage})`}}>
            <h2>Decorations</h2>
          </div>
          <Typography variant="h6" style={{marginLeft: '5%', alignSelf: 'flex-start'}}>Menu</Typography>
          <div className={classes.menu}>
            {data.data.menu.filter(item => item.category === 'Decorations').map(item => (
